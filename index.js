...
function filterUnfundedOnly() {
    ...
    let listNotMet = GAMES_JSON.filter ((game)=>{
        return game.pledged < game.goal;
    });

    // use the function we previously created to add the unfunded games to the DOM
    addGamesToPage(listNotMet)
}
...
function filterFundedOnly() {
    ...
    let listMet = GAMES_JSON.filter ((game)=>{
        return game.pledged > game.goal;
    });
...
const displayStr = `A total of $${totalRaised.toLocaleString("en-us")} has been raised for ${GAMES_JSON.length} game${numUnfunded!=1 ? "s" : ""}. Currently, ${numUnfunded} game${numUnfunded!=1 ? "s" : ""} remain${numUnfunded!=1 ? "" : "s"} unfunded. We need your help to fund these amazing games!`
...
runnerupPledge.textContent = second.name;
